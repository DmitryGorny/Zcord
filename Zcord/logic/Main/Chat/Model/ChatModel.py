...
class ChatModel:
    ...
    def ask_for_cached_messages(self):
        message_client.MessageConnection.send_service_message(f"__CACHED-REQUEST__")

    def send_message(self, text: str):
        message_client.MessageConnection.send_message(text)

    def send_friend_request(self, chat_id, friend_nick):
        message_client.MessageConnection.send_message(f"__FRIEND-ADDING__&{friend_nick}")
    ...
    def accept_friend_request(self, user, friend_nick):
        ...
        if friendAdding.acceptRequest(friend_nick):
            message_client.MessageConnection.send_message(f"__ACCEPT-REQUEST__&{friend_nick}")
        else:
    ...
    def reject_request(self, user, friend_nick, deleteFriend:bool = False):
        ...
        if friendAdding.rejectRequest(friend_nick, deleteFriend):
            message_client.MessageConnection.send_message(f"__REJECT-REQUEST__&{friend_nick}")
        else:
            print("Ошибка целостности бд: в таблице friendship не существует запроса в друзья")
    ...
    def block_user(self, user, friend_nick):
        ...
        message_client.MessageConnection.send_message(f"__DELETE-REQUEST__&{friend_nick}")
