...
class Client:
    def __init__(self, user_id: str, nick, writer: asyncio.StreamWriter = None):
        self._id = user_id
        self._nick = nick
        self._writer = writer
    ...
    def friends(self, friends: List[Dict[str, str]]) -> None:
        for friend_attrs in friends:
            fr = Friend(friend_attrs['id'], friend_attrs['nickname'], friend_attrs['chat_id'], friend_attrs['status'])
            self.__friends[friend_attrs['status']] = fr
...
class Friend(Client):
    def __init__(self, user_id: str, nick, chat_id: str, friendship_status: str):
        super(Friend, self).__init__(user_id, nick)
