...
class FriendRequestListModel(QObject):
    ...
    def decline_request(self, user_id: str) -> None:
        try:
            ClientConnections.send_service_message(msg_type="DECLINE_FRIEND", extra_data={'user_id': user_id})
    ...
    def accept_request(self, user_id: str) -> None:
        try:
            ClientConnections.send_service_message(msg_type="ACCEPT-FRIEND", extra_data={'receiver_id': user_id,
                                                                                         'sender_id': str(
                                                                                             self._user.id)})
            self.remove_others_request_view.emit(user_id)
        except Exception as e:
